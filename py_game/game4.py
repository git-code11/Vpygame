...
class Game_box(GameObject):
	...
	def collide_one(self,obj):
		if(self == obj):
			raise Exception("Same Value Included")
		return self.abs_rect.colliderect(obj.abs_rect)
...
class Player(GameObject):
	...
	def collide_with_ball(self,_ball=0):
		...
		if(not _ball):
			_ball = self.the_ball
		if(self._bart.abs_rect.colliderect(_ball.rect)):
		...
		elif(self.has_gun and (self.gun_1.abs_rect.colliderect(_ball.rect) or self.gun_1.abs_rect.colliderect(_ball.rect)) ):
			if(self.hold_ball_firm):
				_ball.stop()
				_ball.reset(self._bart.abs_rect.centerx,self._bart.abs_rect.top-self.the_ball._size[1]/2)
				#_ball.reset(self._bart.abs_rect.centerx,self._bart.abs_rect.top)
...
def game_window():
	...
	box_size = (10,10)
	player_width = 15
	#my_box_1 = Game_box(_margin=_margin,_size=_size,_color=BLUE,_background=0)
	#my_box_1.use_rect()
